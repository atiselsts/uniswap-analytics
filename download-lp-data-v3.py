#!/usr/bin/env python

#
# This file creates a local database of:
# - all Uniswap v3 burns and mints
# and stores them in CSV files on the disk.
#
# Attention: Google BigQuery access is required!
# Only Ethereum supported for now.
# (Polygon DB also exists and could be easily queried instead.)
#

import os
from web3 import Web3
from google.cloud import bigquery
import pandas as pd
from datetime import date, timedelta, datetime

UNISWAP_VERSION = 3

DIR = os.path.join("data", f"uniswap-v{UNISWAP_VERSION}-lp")

YEAR = os.getenv("YEAR")
if YEAR is None or len(YEAR) == 0:
    YEAR = "2023"
YEAR = int(YEAR)

START_DATE = date(YEAR, 4, 1)
if YEAR == date.today().year:
    END_DATE = date.today() - timedelta(days=1)
else:
    END_DATE = date(YEAR, 12, 31)

V3_MINT_TOPIC = "0x7a53080ba414158be7ec69b987b5fb7d07dee101fe85488f0853ae16239d0bde"
V3_BURN_TOPIC = "0x0c396cd989a39f4459b5fa1aed6a9a8dcdbc45908acfd67e028cd568da98982c"


MINT_V3_JS_CODE = """
    let parsedEvent = {
	"anonymous": false,
	"inputs": [{
		"indexed": false,
		"internalType": "address",
		"name": "sender",
		"type": "address"
	}, {
		"indexed": true,
		"internalType": "address",
		"name": "owner",
		"type": "address"
	}, {
		"indexed": true,
		"internalType": "int24",
		"name": "tickLower",
		"type": "int24"
	}, {
		"indexed": true,
		"internalType": "int24",
		"name": "tickUpper",
		"type": "int24"
	}, {
		"indexed": false,
		"internalType": "uint128",
		"name": "amount",
		"type": "uint128"
	}, {
		"indexed": false,
		"internalType": "uint256",
		"name": "amount0",
		"type": "uint256"
	}, {
		"indexed": false,
		"internalType": "uint256",
		"name": "amount1",
		"type": "uint256"
	}],
	"name": "Mint",
	"type": "event"
    }
    return abi.decodeEvent(parsedEvent, data, topics, false);
"""

MINT_V3_QUERY = """
CREATE TEMP FUNCTION
  PARSE_LOG(data STRING, topics ARRAY<STRING>)
  RETURNS STRUCT<`sender` STRING, `owner` STRING, `tickLower` STRING, `tickUpper` STRING, `amount` STRING, `amount0` STRING, `amount1` STRING>
  LANGUAGE js AS \"\"\"{0}\"\"\"
OPTIONS
  ( library="https://storage.googleapis.com/ethlab-183014.appspot.com/ethjs-abi.js" );

WITH parsed_logs AS
(SELECT
    address
    ,logs.block_timestamp AS block_timestamp
    ,logs.block_number AS block_number
    ,logs.transaction_hash AS transaction_hash
    ,logs.log_index AS log_index
    ,PARSE_LOG(logs.data, logs.topics) AS parsed
FROM `bigquery-public-data.crypto_ethereum.logs` AS logs
WHERE
  DATE(block_timestamp) = '{1}'
  AND topics[SAFE_OFFSET(0)] = '{2}'
)
SELECT
    block_timestamp
    ,block_number
    ,transaction_hash
    ,address
    ,parsed.tickLower AS `tickLower`
    ,parsed.tickUpper AS `tickUpper`
    ,parsed.amount AS `liquidity`
    ,parsed.amount0 AS `amount0`
    ,parsed.amount1 AS `amount1`
FROM parsed_logs
ORDER BY block_timestamp, log_index ASC
"""


BURN_V3_JS_CODE = """
    let parsedEvent = {
	"anonymous": false,
	"inputs": [{
		"indexed": true,
		"internalType": "address",
		"name": "owner",
		"type": "address"
	}, {
		"indexed": true,
		"internalType": "int24",
		"name": "tickLower",
		"type": "int24"
	}, {
		"indexed": true,
		"internalType": "int24",
		"name": "tickUpper",
		"type": "int24"
	}, {
		"indexed": false,
		"internalType": "uint128",
		"name": "amount",
		"type": "uint128"
	}, {
		"indexed": false,
		"internalType": "uint256",
		"name": "amount0",
		"type": "uint256"
	}, {
		"indexed": false,
		"internalType": "uint256",
		"name": "amount1",
		"type": "uint256"
	}],
	"name": "Burn",
	"type": "event"
    }
    return abi.decodeEvent(parsedEvent, data, topics, false);
"""


BURN_V3_QUERY = """
CREATE TEMP FUNCTION
  PARSE_LOG(data STRING, topics ARRAY<STRING>)
  RETURNS STRUCT<`sender` STRING, `tickLower` STRING, `tickUpper` STRING, `amount` STRING, `amount0` STRING, `amount1` STRING>
  LANGUAGE js AS \"\"\"{0}\"\"\"
OPTIONS
  ( library="https://storage.googleapis.com/ethlab-183014.appspot.com/ethjs-abi.js" );

WITH parsed_logs AS
(SELECT
    address
    ,logs.block_timestamp AS block_timestamp
    ,logs.block_number AS block_number
    ,logs.transaction_hash AS transaction_hash
    ,logs.log_index AS log_index
    ,PARSE_LOG(logs.data, logs.topics) AS parsed
FROM `bigquery-public-data.crypto_ethereum.logs` AS logs
WHERE
  DATE(block_timestamp) = '{1}'
  AND topics[SAFE_OFFSET(0)] = '{2}'
)
SELECT
    block_timestamp
    ,block_number
    ,transaction_hash
    ,address
    ,parsed.tickLower AS `tickLower`
    ,parsed.tickUpper AS `tickUpper`
    ,parsed.amount AS `liquidity`
    ,parsed.amount0 AS `amount0`
    ,parsed.amount1 AS `amount1`
FROM parsed_logs
ORDER BY block_timestamp, log_index ASC
"""


def get_data(client, date, name, query):
    year = date[:4]
    filename = os.path.join(DIR, year, date + "-" + name + ".csv")
    if os.access(filename, os.R_OK):
        print(f"file {filename} already exists")
        return False

    query_job = client.query(query)
    iterator = query_job.result(timeout=300)
    with open(filename, "w") as f:
        s = ["timestamp", "block", "pool", "tickLower", "tickUpper", "liquidity", "amount0", "amount1", "tx_hash"]
        f.write(",".join(s) + "\n")

        for row in iterator:
            timestamp = int(round(row[0].timestamp()))
            block = row[1]
            tx_hash = row[2]
            pool = row[3]
            tickLower = row[4]
            tickUpper = row[5]
            liquidity = row[6]
            amount0 = row[7]
            amount1 = row[8]
            s = [str(u) for u in [timestamp, block, pool, tickLower, tickUpper, liquidity, amount0, amount1, tx_hash]]
            f.write(",".join(s) + "\n")
    return True


def get_v3_mints(client, date):
    query = MINT_V3_QUERY.format(MINT_V3_JS_CODE, date, V3_MINT_TOPIC)
    return get_data(client, date, "mints", query)


def get_v3_burns(client, date):
    query = BURN_V3_QUERY.format(BURN_V3_JS_CODE, date, V3_BURN_TOPIC)
    return get_data(client, date, "burns", query)


def main():
    os.makedirs(os.path.join(DIR, str(YEAR)), exist_ok=True)

    client = bigquery.Client()
    end_date = END_DATE
    if end_date is None:
        end_date = datetime.today() - timedelta(days=1)
    dates = pd.date_range(START_DATE, end_date, freq='d')
    dates = [d.strftime('%Y-%m-%d') for d in dates]
    for d in dates:
        print(d)
        get_v3_mints(client, d)
        get_v3_burns(client, d)


if __name__ == "__main__":
    main()
    print("all done")
